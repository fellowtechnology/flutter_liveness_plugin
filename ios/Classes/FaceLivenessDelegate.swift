...
class LivenessModel {
    ...
    private var faceLivenessDelegate: FaceLivenessDelegate?;

    init(
        faceLivenessDelegate: FaceLivenessDelegate  
    ) {
        self.faceLivenessDelegate = faceLivenessDelegate;
    }
    ...
    func receive(image: UIImage) {
        ...
        if counter > 200 {
            if let faceLivenessDelegate = self.faceLivenessDelegate {
                faceLivenessDelegate.onDetectOver(image: image, scores: "OK");
            }
...
class FaceLivenessDelegate {
    ...
    func onDetectOver(image: UIImage?, scores: String) {
        if let image = image {
